...
### Experimental support for streaming output

If you want to see the build output as it is produced, you can enable experimental streaming support by setting an environment variable:

    $ export STREAMING_BUILD_RESULT=true

and run `heroku deploy` again. The output should now be printed in real time as it is generated by the build service.
